...
class SparsePolynomial : public sparse_vector_t {
 public:
  ...
  bool IsEqual(const Polynomial&, const double = EPS) const;
  double Eval_monomios_0 (const double) const;
  void MonomiosIgual_0();
};
...
void SparsePolynomial::MonomiosIgual_0() {
int indice_polinomio = 0;
int indice_polinomio_anterior = 0;
  for(int i = 0; i < get_nz(); i++) {
    if (i == 0 && at(i).get_inx() >= 0) {
      indice_polinomio = at(i).get_inx();
      while(indice_polinomio >= 0) {
        indice_polinomio--;
        if(indice_polinomio == 0) {
          std::cout << "monomio de grado 0  ";
        } else {
        std::cout << " x^" << indice_polinomio << "  ";
        }
      }
    }
    indice_polinomio_anterior = i;
    if (i != 0 && at(i).get_inx() - indice_polinomio_anterior > 1) {
      indice_polinomio = at(i).get_inx();
      while(indice_polinomio - indice_polinomio_anterior > 1) {
        indice_polinomio--;
        std::cout << "x^" << indice_polinomio << "  ";
      }
    }
...
// Practica-3-AyEDdisperso FASE 3
...
/* double SparsePolynomial::Eval_monomios_0 (const double x) const {
    double result{0.0};
  for(int i = 0; i < get_nz(); i++) {
    result += (at(i).get_val() * pow(x, at(i).get_inx())); 
}
...
// Modificación de clase: 
// Para un SparsePolynomial, mostrar los monomios cuyos coeficientes sean 0, probar para sp1, sp2,sp3
// Que se muestre, 0 x², 0 x³ ...
